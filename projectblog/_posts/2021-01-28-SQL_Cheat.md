...
Postgre has extensions of the normal `WHERE column LIKE '%mystring%'` with wildcards (%,_) etc. capabilities. Additional features are stemming, fuzzy matching (spelling mistakes), ranking.
...
Creating a table schema works with `name` `data-type`. ARRAYS can be created with square brackets. Multiple square brackets are nested arrays.
