...
The analysis below was inspired in parts by the geophysicist Yohanes Nuwara (<a href="https://github.com/yohanesnuwara/volve-machine-learning" target="_blank">see here</a>). He also wrote a TDS article <a href="https://towardsdatascience.com/prediction-of-p-sonic-log-in-the-volve-oil-field-using-machine-learning-9a4afdb92fe8" target="_blank">see here</a> on the topic, that's worth your time if you want to dive in deeper!

For more information about this open data set and how to obtain the original version visit <a href="https://www.equinor.com/en/how-and-why/digitalisation-in-our-dna/volve-field-data-village-download.html" target="_blank">Equinor's Data Village</a>.
The full dataset contains ~ 40,000 files!

The full repository including Jupyter Notebook, data, and results can be found **<a href="https://github.com/ChristianHallerX/DataScienceProjects/tree/master/Equinor_Volve_LogML" target="_blank">here</a>**.
{:.note}
...
print('Samples before outliers removal:', len(train_fonly))
~~~
>Samples before outliers removal: 24403


### Method 1: StDev Method (traditional)
...
print('Remaining samples Standard Deviation removal:', len(train_stdev))
~~~
>Remaining samples Standard Deviation removal: 24101
...
print('Remaining samples Isolation Forest removal:', len(train_iso))
~~~
>Remaining samples Isolation Forest removal: 12202
...
print('Remaining samples Min. Covariance removal:', len(train_ee))
~~~
>Remaining samples Min. Covariance removal: 21962
...
print('Remaining samples Outlier Factor removal:', len(train_lof))
~~~
>Remaining samples Outlier Factor removal: 17082


### Method 5: One-class Support Vector Machine (SVM)
...
print('Remaining samples Outlier Factor removal:', len(train_svm))
~~~
>Remaining samples Outlier Factor removal: 21964
...
plt.subplot(3,2,1)
train_trans[train_fonly_names].boxplot()
plt.title('Before', size=15)

plt.subplot(3,2,2)
train_stdev[train_fonly_names].boxplot()
plt.title('Method 1: Standard Deviation Filter', size=15)

plt.subplot(3,2,3)
train_iso[train_fonly_names].boxplot()
plt.title('Method 2: Isolation Forest', size=15)

plt.subplot(3,2,4)
train_ee[train_fonly_names].boxplot()
plt.title('Method 3: Min. Covariance', size=15)

plt.subplot(3,2,5)
train_lof[train_fonly_names].boxplot()
plt.title('Method 4:Local Outlier Factor', size=15)

plt.subplot(3,2,6)
train_svm[train_fonly_names].boxplot()
plt.title('Method 5:One-class SVM', size=15)

plt.tight_layout()
plt.show()
~~~

<br><img src="/assets/img/research/Equinor_Volve_LogML/boxplot.jpg" alt="boxplot" style="width:640px"><br>
